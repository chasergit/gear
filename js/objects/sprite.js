// СПРАЙТ С СОРТИРОВКОЙ ПО ДИСТАНЦИИ К КАМЕРЕ
...
geometry.setAttribute("position",new THREE.Float32BufferAttribute(new Float32Array([-0.5,0.5,0,-0.5,-0.5,0,0.5,0.5,0,0.5,-0.5,0]),3));
geometry.setAttribute("uv",new THREE.Float32BufferAttribute(new Float32Array([0,1,0,0,1,1,1,0]),2));
...
geometry.setAttribute("rotation",new THREE.InstancedBufferAttribute(new Float32Array(),2));
...
geometry.setAttribute("soft",new THREE.InstancedBufferAttribute(new Float32Array(),1));
...
// СПРАЙТ БЕЗ СОРТИРОВКИ. БЕЗ ПРОЗРАЧНОСТИ. НАПРИМЕР,ДЛЯ ОСКОЛКОВ 


function sprite_solid_set(){
...
mat["sprite_solid"]=new THREE.ShaderMaterial({
uniforms:{
map:{value:tex["atlas_sprite"]},
tDepth:{value:null},
screen_resolution:{value:null},
...
mesh["sprite_solid"]=new THREE.Mesh(geometry,mat["sprite_solid"]);
mesh["sprite_solid"].frustumCulled=false;
mesh["sprite_solid"].matrixAutoUpdate=false;
mesh["sprite_solid"].updateMatrixWorld=function(){};
mesh["sprite_solid"].onBeforeRender=function(){
mat["sprite_solid"].uniforms.time.value=time;
}
scene.add(mesh["sprite_solid"]);


}


// СПРАЙТ БЕЗ СОРТИРОВКИ. НАХОДИТСЯ ВСЕГДА ПОЗАДИ ДРУГИХ СПРАЙТОВ. ИСПОЛЬЗУЕТСЯ, НАПРИМЕР, ДЛЯ ЛАМПОЧЕК ВПРИТЫК У СТЕНЫ
...
mesh["sprite_far"].onBeforeRender=function(){
mat["sprite_far"].uniforms.time.value=time;
mat["sprite_far"].uniforms.tDepth.value=water_rtt_scene.depthTexture;
}
...
let rotation=new Float32Array(count*2);
let color=new Float32Array(count*4);
let blend=new Float32Array(count);
let soft=new Float32Array(count);
...
item.rotation=new THREE.InstancedBufferAttribute(rotation,2).setUsage(THREE.StreamDrawUsage);
...
item.soft=new THREE.InstancedBufferAttribute(soft,1).setUsage(THREE.StreamDrawUsage);
