...
let delta_total=Math.min(delta_max,delta);
let delta_chunks=new Array(parseInt(delta_total/delta_frame)).fill(delta_frame);
let delta_rest=delta_total%delta_frame;
if(delta_rest>0){ delta_chunks.push(delta_rest); }


for(let n=0;n<delta_chunks.length;n++){


let found=-1;
for(let i=0;i<delta_length;i++){
if(delta_chunks[n]>=delta_settings[i].ms){ found=i; }
}


if(found>=0){
let steps=delta_settings[found].steps;
delta_time=delta_chunks[n]/steps;
...
debug_start("sprite");
sprites_update([sprite["wolf"],sprite["soul"],sprite["other"],sprite["blood"],sprite["weapon_smoke"],sprite["weapon_muzzle_flash"],sprite["planets"],sprite["muzzle_spark"]],true,"sprite");
debug_end("sprite",mesh["sprite"].geometry._maxInstanceCount);
debug_start("sprite_solid");
sprites_update([sprite["debris"]],false,"sprite_solid");
debug_end("sprite_solid",mesh["sprite_solid"].geometry._maxInstanceCount);
...
mesh["soldier_attack_1"].position.z+=0.037;
if(mesh["soldier_attack_1"].position.z>20){
mesh["soldier_attack_1"].position.z=-6;
}
