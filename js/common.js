...
const radian_to_degrees=180/Math.PI;
const PI_half=Math.PI/2;
const PI=Math.PI;
const PI_3=Math.PI*1.5;
...
let vector3_2=new THREE.Vector3();
let vector3_3=new THREE.Vector3();
let vector3_4=new THREE.Vector3();
let vector4=new THREE.Vector4();
let matrix2=new THREE.Matrix2();
let matrix3=new THREE.Matrix3();
let matrix4=new THREE.Matrix4();
let quaternion=new THREE.Quaternion();
let quaternion_2=new THREE.Quaternion();
...
let delta_frame=0.016; // ЧАСТОТА ОДНОГО КАДРА. МОЖНО 0.016 ВМЕСТО 0.0165. ОСТАЛЬНОЕ ОБРЕЖЕТСЯ В delta_settings 
let delta_max=0.05; // МАКСИМАЛЬНОЕ ЗНАЧЕНИЕ delta
// ЗДЕСЬ КОЛИЧЕСТВО ШАГОВ ФИЗИКИ С УЧЁТОМ delta_chunks. ЕСЛИ delta_chunks=0.001, А У НАС ms:0.002, ТО ПРОПУСТИТ ТАКОЕ МАЛЕНЬКОЕ ЗНАЧЕНИЕ 
let delta_settings=[{ms:0.002,steps:1},{ms:0.006,steps:5},{ms:0.010,steps:5},{ms:0.016,steps:5}];
let delta_length=delta_settings.length;
